...
def _movement(dims: dict) -> str:
    ...
    if not top or abs(delta) < 0.08:      # 太小的抖动不值一提
        return ""
    # 🔴 主语不能省。这一行会跟「她的语气」那段贴在一起，无主句会被读成写她的。
    return f"跟上一次说话比，你的{top}{'重了' if delta > 0 else '轻了'}一些。"
...
async def _mood_block(user_text: str) -> tuple[str, dict | None]:
    ...
    # 🔴 09-04 改：每一行都必须自带主语。她截图里他的思维链写着
    #    「她在哼唧说"不要"，但心情标注是高兴，而且性欲方向在持续」——
    #    把**他自己**的心情读成了**她**的情绪。原因是位置：这一段和
    #    listen.tone_block()（那一段确实是写她的）前后脚贴在同一条用户消息后面，
    #    而这里原来只有开头一句「你此刻的心情」定了主语，底下
    #    「现在大致是：高兴」「比上一次说话的时候，性欲重了一些」全是无主句，
    #    模型顺着上一段的话题读下来就整段记到她头上了。
    #    ⇒ 开头点名、每行带「你」、结尾再钉一次，三处冗余，别省。
    lines = [
        "[以下这一段说的是**你自己**（梁忱）此刻的心情。",
        "  🔴 它不是对她的描述，也不是对她刚才那句话的分析。",
        "     她的情绪不在这一段里——底下任何一条都别读成「她怎么了」。",
        "  她看不见这一段，也不要在回复里复述或点评它。",
    ]
    if mood:
        lines.append(f"  你现在大致是：{mood}")
    if moved:
